...
(SELECT DISTINCT certname,status,resource_type,resource_title
    FROM (SELECT DISTINCT ON (certname, resource_type,resource_title, property) *
        FROM
        (SELECT reports.certname AS certname,
            ...
            WHERE (timestamp > '2001-01-01') order by timestamp) foo) bar) baz
