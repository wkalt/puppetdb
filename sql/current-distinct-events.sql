...
(SELECT certnames.certname,
  ...
  AND resource_events.timestamp <= '2015-06-30 17:00:00.0'
  GROUP BY certname, resource_type, resource_title, property)
SELECT resource_events.message, resource_events.old_value,
reports.receive_time AS report_receive_time, resource_events.containment_path,
reports.certname, trim(leading '\x' from reports.hash::text) AS report,
resource_events.timestamp, environments.name AS environment, reports.configuration_version,
resource_events.new_value, resource_events.resource_title, resource_events.status,
resource_events.property, resource_events.resource_type, resource_events.line,
reports.end_time AS run_end_time, resource_events.containing_class,
resource_events.file, reports.start_time AS run_start_time
FROM resource_events
