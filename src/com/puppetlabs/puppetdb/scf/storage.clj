...
(defn db-deprecated?
  ...
  (when (and (sutils/postgres?)
             ...
             (not (and (= "pe-puppetdb" product-name)
...
(defn warn-on-db-deprecation
  ...
  [product-name]
  (when-let [deprecated-message (db-deprecated? product-name)]
...
(defn validate-database-version
  "Checks to ensure that the database is supported, fails if supported, logs
   if deprecated"
  [product-name action-for-unsupported-fn]
  (fail-on-unsupported action-for-unsupported-fn)
  (warn-on-db-deprecation product-name))
