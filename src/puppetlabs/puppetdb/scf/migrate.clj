...
(defn consolidate-numeric-values-to-numeric
  []
  (let [hash-type (if (sutils/postgres?) "bytea" "varchar(40)")
    ...
    (sql/do-commands
      (str "INSERT INTO fact_values_transform
            (id, value_hash, value_type_id, value_numeric,
            value_string, value_boolean, value)
            SELECT id, " (munge-hash "value_hash") ", value_type_id,
              coalesce(value_integer, value_float), value_string,
    ...
    (sql/do-commands
      "ALTER TABLE fact_values_transform RENAME TO fact_values"
      "CREATE INDEX fact_values_value_numeric_idx ON fact_values(value_numeric)"
      "ALTER TABLE fact_values ADD CONSTRAINT fact_values_value_hash_key UNIQUE (value_hash)"
      "ALTER TABLE fact_values ADD CONSTRAINT fact_values_pkey PRIMARY KEY (id)"
...
(def migrations
  ...
  {1 initialize-store
   ...
   38 consolidate-numeric-values-to-numeric
   })
