...
(defn anonymize-resource-event
  ...
  (let [newcontext {"node"          (get context "node")
    ...
    (-> event
        ...
        (update-in     ["new_value"]        anonymize-leaf :parameter-value (assoc newcontext :parameter-value (get event "new_value")) config)
        (update-in     ["old_value"]        anonymize-leaf :parameter-value (assoc newcontext :parameter-value (get event "old_value")) config)
        (update-in     ["resource_type"]    anonymize-leaf :type newcontext config)
        (update-in-nil ["file"]             anonymize-leaf :file newcontext config)
        (update-in-nil ["line"]             anonymize-leaf :line newcontext config)
        (update-in-nil ["containment_path"] anonymize-containment-path newcontext config))))
...
(defn anonymize-catalog
  ...
  (let [context {"node" (get catalog ["name"])}]
    (-> catalog
        ...
        (update-in ["transaction_uuid"] anonymize-leaf :transaction-uuid context config)
